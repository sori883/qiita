...
tags:
  ...
  - AWSCDK
private: false
updated_at: '2026-02-01T01:50:10+09:00'
...
# はじめに
未知のAWS環境へ`cdk deploy`することになった経験はありますか？  
私は何度かあります。  

この世界ではドキュメントがなくよく分からない状態、でもスケジュールがあるから来週には改修対応してデプロイしなければ行けないという厄災が起きます。  
この記事では、今までに身につけた未知のAWS環境へデプロイする時に既存ドキュメントに頼らずなるはやでデプロイ方法をまとめます。  
...
ルートテーブルとかセキュリティグループの変更はなかなか気付けないので絶対に実施することをおすすめします。  
...
私は最後の最後、天に祈りながら`cdk deploy`しています。  

# もしデプロイエラ−が出たら
CloudFormationにはロールバック機能があるため、大抵は自動ロールバックで元通りになるはずです。  
もし`UPDATE_FAILED`状態になってスタック更新も出来なくなったら落ち着いて下記を対応しましょう。  

**CloudFormationスタックのロールバック**をする。  
以下コマンドを実行し、ロールバックを再試行してみます。  
VPC Endpointといった時間かかる系は少し待って再試行することで解決したりします。  

```
aws cloudformation continue-update-rollback --stack-name <スタック名>
```

**失敗リソースを無視してロールバック**をする。  
失敗リソースを無視してロールバックすることで、ロールバック完了させることが出来ます。  
今のところ、この方法でロールバック出来なかったことはありません。最後の切り札。  

```
aws cloudformation continue-update-rollback \
 --stack-name <スタック名> \
 --resources-to-skip <失敗したリソースの論理ID#1> <失敗したリソースの論理ID#2>
```
